...
{ 
	...
	{ 
		...
		print_matrix(rGraph,max_flow,V);
	} 
...
{ 
    ...
    fd.open("simple_graph2.txt", ios::in);
	...
	print_matrix(graph,0,V);
	unsigned long long t0,t1;
    t0=rdtsc();     //start timer 

	cout << "The maximum possible flow is " << fordFulkerson(graph, s-1, t-1, V) << endl;

	t1=rdtsc();     //stop timer

    cout << "Time taken: " <<(t1-t0)/FREQ << endl; //print time taken to find max flow
	return 0; 
}
